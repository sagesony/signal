...
import { Bookmark, BookmarkCheck, Flame, Maximize2 } from "lucide-react"
...
export function AdCard({ ad, onSaveToggle }: AdCardProps) {
  ...
  function handleSaveChange(saved: boolean) {
    setIsSaved(saved)
    onSaveToggle?.(ad.id, saved)
  }
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "rounded-xl border bg-card overflow-hidden card-hover group cursor-pointer",
      ...
      >
        ...
        <div className="relative aspect-[4/3] bg-muted overflow-hidden">
          ...
          {/* Save quick-action (stop propagation so it doesn't open modal) */}
          <div
            className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => handleSaveChange(!isSaved)}
              className={cn(
